...
def maiorComponente(grafo):
    ...
    for componente in componentes:
        ...
        # {numero de vertices: componente}
        subGrafos[subGrafo.size()] = subGrafo
...
def gerarPng(grafo, nome):
    ...
    # Experimente diferentes layouts conforme necessário
...
def visualizarParticao(grafo, partitions, nome):
    ...
    fig, ax = plt.subplots()
    ax.scatter(*zip(*pos.values()), s=100, c=cores,
...
def questao4(grafo):
    ...
    print(
        f'Grafo com {grafo.number_of_nodes()} vertices, e {grafo.number_of_edges()} arestas')
    print(
        f'Maior componente conectado: {maior.number_of_nodes()} vertices, e {maior.number_of_edges()} arestas')
...
def questao5(grafo):
    ...
    print(
        "Modularidade esperada em um grafo aleatorio NÃO ponderado e NÃO direcionado:[-0.5,1]")
    print(
        f"Modularidade encontrada na partição gerada pelo algoritmo de Louvain: {modularidade}")
...
def deptComunidade(grafo, particao):
    ...
    comunidades = {
        comunidade: departamentos for comunidade in range(len(particao))}
...
def questao8(grafo):
    ...
    dataFrame = pd.DataFrame(
        porcentagens, columns=departamentos, index=comunidades)
    ...
    plt.xticks(range(len(dataFrame.columns)),
               dataFrame.columns, rotation='vertical')
...
def questao9(grafo):
    ...
    print(
        f'Quantidade de cliques maximais com a maior ordem:{len(maioresCliques)}')
...
def questao10(grafo):
    ...
    visualizarParticao(grafoCliques, list(
        maioresCliques), "MaioresCliquesMaximais")
